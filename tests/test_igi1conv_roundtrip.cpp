...
TEST_F(IGI1ConvTest, VersionFlagReportsOneNineZero) {
    std::string out;
    EXPECT_EQ(RunIGI1Conv("--version", &out), 0);
    EXPECT_NE(out.find("1.9.0"), std::string::npos) << "got: " << out;
}

TEST_F(IGI1ConvTest, HelpReportsOneNineZero) {
    std::string out;
    EXPECT_EQ(RunIGI1Conv("--help", &out), 0);
    EXPECT_NE(out.find("v1.9.0"), std::string::npos) << "got: " << out;
}
